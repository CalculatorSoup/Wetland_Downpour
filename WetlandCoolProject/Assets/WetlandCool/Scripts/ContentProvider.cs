...
namespace FSCStage.Content
{
    ...
    {
        public string identifier => FSCStage.GUID + "." + nameof(ContentProvider);
        ...
        public IEnumerator LoadStaticContentAsync(LoadStaticContentAsyncArgs args)
        {
            ...
            yield return LoadAssetBundle(
                Path.Combine(assetsFolderFullPath, FSCContent.ScenesAssetBundleFileName),
                args.progressReceiver,
                (assetBundle) => scenesAssetBundle = assetBundle);

            AssetBundle assetsAssetBundle = null;
            yield return LoadAssetBundle(
                Path.Combine(assetsFolderFullPath, FSCContent.AssetsAssetBundleFileName),
                args.progressReceiver,
                (assetBundle) => assetsAssetBundle = assetBundle);

            yield return FSCContent.LoadAssetBundlesAsync(
